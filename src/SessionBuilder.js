      ...
      return this.storage.isTrustedIdentity(
        ...
        return this.storage.loadSession(address).then(function(serialized) {
          ...
          return Promise.all([
            ...
            this.storage.saveIdentity(this.remoteAddress.toString(), session.indexInfo.remoteIdentityKey).then(function(changed) {
              if (changed) {
  ...
  processV3: function(record, message) {
    ...
    return this.storage.isTrustedIdentity(
        ...
        return this.initSession(false, preKeyPair, signedPreKeyPair,
            ...
            return this.storage.saveIdentity(this.remoteAddress.toString(), message.identityKey.toArrayBuffer()).then(function(changed) {
              if (changed) {
                return record.removePreviousSessions();
              }
            }).then(function() {
